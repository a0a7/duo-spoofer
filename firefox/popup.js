...
// ============= OPTIONS/DATA MANAGEMENT FUNCTIONALITY =============

async function showOptionsDeleteModal(prompt, onAccept = () => {}) {
  if (window.confirm(prompt)) {
    onAccept();
  }
}

// Set up feedback element
const optionsFeedback = document.getElementById("optionsFeedback");

function showOptionsFeedback(message, isError = false) {
  optionsFeedback.innerHTML = message;
  optionsFeedback.style.color = isError ? "red" : "green";
...
document.getElementById("importButton").addEventListener("click", async function () {
  ...
  if ((await getDeviceInfo()).devices?.length > 0) {
    showOptionsDeleteModal(`This will replace all data! Continue?`, () => {
  ...
  reader.onload = async (e) => {
    ...
    try {
      ...
      for (let index in newData.devices) {
        ...
        showOptionsFeedback(`Verifying ${Number(index) + 1} / ${newData.devices.length}...`);
        // Grab that data
      ...
      showOptionsFeedback(`Updating data...`);
      await setDeviceInfo(newData);
      if (invalidDevices > 0) {
        showOptionsFeedback(`Data imported, but ${invalidDevices} device${invalidDevices == 1 ? "" : "s"} failed validation`);
      } else {
        showOptionsFeedback("Data imported!");
      }
      // Update the popup display
      await updatePage(await getDeviceInfo());
    } catch (err) {
      ...
      setDeviceInfo(ogData).then(() => {
        ...
        showOptionsFeedback(`Invalid data`, true);
      }).catch(e => {
        console.error("Failed to set back to original data", e);
        // If the OG data had devices
        if (ogData.devices?.length > 0) {
          showOptionsFeedback(`Failed to set back to old data${ogData.devices?.length > 0 ? '. Reimport this later!' : ''}`, true);
          downloadFile(btoa(JSON.stringify(ogData)), "auto-2fa.txt");
        } else {
          showOptionsFeedback(`Failed to verify data`, true);
        }
      });
    } finally {
      importFile.value = "";
...
document.getElementById("exportButton").addEventListener("click", async function () {
  if ((await getDeviceInfo()).devices?.length > 0) {
    ...
    showOptionsFeedback(`Data exported!`);
  } else {
    showOptionsFeedback(`No data to export!`, true);
  }
});
...
document.getElementById("exportTOTPButton").addEventListener("click", async function () {
  ...
  if (!totps.length) {
    showOptionsFeedback("No devices have TOTP data!", true);
  } else {
    downloadFile(totps.join("\n"), "duo-mobile-totps.txt");
    if (totps.length == info.devices.length) {
      showOptionsFeedback(`Exported all devices`);
    } else {
      showOptionsFeedback(`Exported ${totps.length} of ${info.devices.length}`);
    }
  }
});
...
document.getElementById("resetButton").addEventListener("click", () => {
  showOptionsDeleteModal(`Are you sure you want to delete all data?`, async () => {
    await clearAll();
    showOptionsFeedback("Data reset");
    // Update the popup display
    await updatePage(await getDeviceInfo());
    changeScreen("main");
  });
